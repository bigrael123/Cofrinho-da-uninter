	...
	public static void main(String [] args) { 
		...
		while(true) { //loop da ação inicial
			...
			try {
				...
				System.out.println("Por favor digite apenas numeros"); //erro quando usário digita algo que não é um número
				continue;
			}
			...
			if(a >0 && a<6) {  //se o usuário tiver digitado um número entre 1 e 5 continue o if
								...
								if(aT == 1) { //se o usuário tiver digitado o número 1 continue o if
									while(true) { //while para adicionar reais
										...
										try  {
											...
											int aV = adicionarValor.nextInt(); //da o valor de aV o que o usário digitou
											System.out.println("Você adicionou "+ aV + " Reais."); //escreve uma mensagem com a quantidade de reais adicionados
											...
											System.out.println("Por favor digite apenas numeros."); //erro se o usuário digitar algo que não seja numers
											continue; //reinicia o loop da adição de reais
										}
									}
								break; // encerra o loop do tipo de moeda
								}
							//os próximos 2 ifs são iguais ao amterior mas com euro e dolar ao invés de reais
							if(aT == 2) {
								while(true) {
								...
								System.out.println("Numero inválido."); //mensagem de erro se o numero estiver fora do intervalo
								continue;
							}
						}
						catch(Exception e) {
							System.out.println("Por favor digite apenas numeros."); //erro se o usuário digitar algo que nãp seja numeros
					...
					while(true) { //while da remoção de moeda
						...
						try {
							...
							if(rM >0 && rM-1<listaMoedas.size()) { //checa se o número esta no intervalo do array
								remover(listaMoedas.get(rM-1)); //seleciona a moeda digitada pelo usuário( é o valor digitado -1 para achar a moeda)
							}
							else {
								if(rM == listaMoedas.size()+1) { //s o usuário tiver digitado o número voltar(1 a mais que o tamanho da lista)
							...
							System.out.println("Por favor digite apenas numeros."); //se o usuário digitar algo que não seja números esse erro será mostrado
				...
				System.out.println("Numero inválido."); //texto mostrado se o usuário digitar um numero que não esteha no intervalo correto
